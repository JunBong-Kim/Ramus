...
주제 : 코로나 대유행 속 모두가 방역의 주체가 되어 안전하고 건강한 도서관 형성에 도움을 주는 종합 어플리케이션
	...
	* 각 기능설명란 아래 그림을 클릭하면 설명과 함께 실제 앱 구동 사진이 첨부되어 있습니다 
...
개발에 사용한 구조와 패턴
	...
	5 서버로는 구글사에서 제공하는 Firebase를 사용했고, 주 기능인 쪽지 기능의 알림은 동사의 FCM을 활용하였습니다.
 
----------------------------------------------------------------------------------------------------------------------

Libraries

	저희 어플내에는  smtp(simple mail transfer protocol) 을 활용한  메일 송신을 위해 Java Mail Api를 사용했습니다.
...
* 앱 구동을 위해서는 안드로이드 스튜디오 환경에서 실행해야합니다.
  다만 sha-1키 등록을 저희 파이어베이스 서버단에서 진행해야해서 이미 등록한 컴퓨터에서 빌드한 앱 외에는 서버에 접근이 불가능합니다.
  만약에 시연을 원하신다면 람머스 팀측에 따로 연락을 주신다면 android 기반 운영체제 상에서 작동가능한 apk 파일을 보내드리겠습니다.
-----------------------------------------------------------------------------------------------------------------------------------
* QR 사용법
  학생 메일 등록 후, QR시트 클릭후 미리 지정한 스트링 값의 QR을 찍고, 시간을 지정하면 등록 됩니다.
